...
public class WheelTime {
     ...
     * @param s
     */
    private void setLunar(final int year, final int month, final int day, boolean isLeap, int h, int m, int s) {
        // 年
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            ...
            public void onItemSelected(int index) {
                ...
                //判断时分 new NumericWheelAdapter(0, h)
                // wv_hours.setAdapter(new NumericWheelAdapter(0,ChinaDate.getMaxHour(year,month,day)));
                // maxItem = ChinaDate.monthDays(year_num, wv_month.getCurrentItem() + 1);
    ...
    private void resetHourMinutes() {
        int currentYearIndex = wv_year.getCurrentItem();
        int currentYear = currentYearIndex + startYear;
        int currentMonthIndex = 0;
        int currentMonth = 0;
        int currentDaysIndex = 0;
        int currentDays = 0;
        ...
        if (currentYear == startYear) {
            ...
            currentMinutesIndex = wv_minutes.getCurrentItem();
            currentMiuntes = currentMinutesIndex;
        }
        ...
        if (currentYear == startYear) {
            if (currentMonth == startMonth) {
                if (currentDays == startDay) {
                    wv_hours.setAdapter(new NumericWheelAdapter(startHour, 23));
                    if (wv_hours.getItemsCount() < currentHourIndex) {
                    ...
                    if (currentHour == startHour) {
                        wv_minutes.setAdapter(new NumericWheelAdapter(startMinutes, 59));
                        if (wv_minutes.getItemsCount() < currentMiuntes) {
            ...
            wv_hours.setAdapter(new NumericWheelAdapter(0, 23));
            // wv_minutes.setCurrentItem(currentHourIndex);
            wv_minutes.setAdapter(new NumericWheelAdapter(0, 59));
        }
    }
...
//                maxItem = endD;
    ...
    public void setRangDate(Calendar startDate, Calendar endDate) {
        ...
        if (startDate == null && endDate != null) {
            ...
            int minutes = endDate.get(Calendar.MINUTE);
            if (year > startYear) {
                this.endYear = year;
                this.endMonth = month;
                this.endDay = day;
            ...
            this.startMinutes = startDate.get(Calendar.MINUTE);
            this.endMinutes = endDate.get(Calendar.MINUTE);
        }
    }
